            ...
            <div id="loginForm" class="space-y-8">
                ...
                <div class="space-y-5">
                    <div class="relative">
                        ...
                        <input type="text" id="loginId" name="userId" onkeyup="handleLoginKeyup(event)" placeholder="아이디 (@gmail.com)" class="w-full bg-white/5 border border-white/10 rounded-2xl py-4.5 pl-14 pr-5 text-white text-lg focus:outline-none focus:border-blue-500 transition-all">
                    </div>
                    <div class="relative">
                        <i data-lucide="lock" class="absolute left-5 top-4.5 w-6 h-6 text-gray-500"></i>
                        <input type="password" id="loginPw" name="password" onkeyup="handleLoginKeyup(event)" placeholder="비밀번호" class="w-full bg-white/5 border border-white/10 rounded-2xl py-4.5 pl-14 pr-5 text-white text-lg focus:outline-none focus:border-blue-500 transition-all">
                    </div>
                </div>
                ...
                <div id="loginLockedArea" class="hidden space-y-4 p-6 bg-red-500/10 border border-red-500/30 rounded-2xl animate-fade-in">
				    <div class="flex items-start gap-3">
				        <i data-lucide="lock" class="w-6 h-6 text-red-500 mt-1"></i>
				        <div>
				            <p class="text-white font-bold text-lg">계정이 잠금되었습니다</p>
				            <p class="text-gray-400 text-sm leading-relaxed">
				                비밀번호 5회 이상 누적 실패로 보안을 위해 계정이 잠금되었습니다. 본인 인증 후 비밀번호를 재설정하여 잠금을 해제하세요.
				            </p>
				        </div>
				    </div>
				    <button onclick="switchTab('find'); setTimeout(() => switchFindMode('PW'), 100);" 
				            class="w-full py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl transition-all shadow-lg">
				        비밀번호 재설정하러 가기
				    </button>
				</div>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div class="col-span-2">
						    ...
						    <div class="flex gap-3">
						        <input type="text" id="regId" placeholder="example@gmail.com" 
						               class="flex-1 bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white text-lg focus:outline-none focus:border-blue-500 transition-all">
						        <button type="button" onclick="checkDuplicateId()" 
						                class="px-6 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl transition-all shadow-lg active:scale-95">
						            중복 확인
						        </button>
						    </div>
						    <p id="idCheckMsg" class="text-xs mt-2 ml-1 hidden"></p>
						</div>
                        <div>
                        ...
                        <div class="col-span-1">
                             ...
                             <input type="text" id="regPhone" oninput="autoHyphen(this)" maxlength="13" placeholder="01X-0000-0000" class="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white text-lg">
                        </div>
                        
                        <div class="col-span-2">
						    <label class="text-blue-400 text-xs font-bold ml-1 mb-1 block">이메일 주소 (Gmail)</label>
						    <div class="flex gap-3">
						        <input type="email" id="regEmail" placeholder="partner@gmail.com" 
						               class="flex-1 bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white text-lg focus:outline-none focus:border-blue-500 transition-all">
						    ...
						    <p id="emailCheckMsg" class="text-xs mt-2 ml-1 hidden"></p>
						</div>
...
<script>
    ...
    var isIdChecked = false; // 중복 확인 완료 여부
    var checkedId = "";      // 중복 확인 완료된 아이디 보관
    var isEmailChecked = false; // [추가] 이메일 중복 확인 완료 여부
    var checkedEmail = "";      // [추가] 중복 확인 완료된 이메일
    
 	// [추가] 페이지 로드 후 아이디 입력 감지 이벤트 등록
    document.addEventListener('DOMContentLoaded', function() {
        var regIdInput = document.getElementById('regId');
        if(regIdInput) {
        ...
        fetch('/account/api/check-duplicate-id?userId=' + encodeURIComponent(userId))
            ...
            .then(function(data) {
                ...
                if (data.success) {
                    ...
                    msgEl.className = "text-xs mt-2 ml-1 text-green-400 font-bold";
                    isIdChecked = true;
                    checkedId = userId;
                } else {
                    showAlert(data.message, 'error');
                    msgEl.innerText = "✕ " + data.message;
                    msgEl.className = "text-xs mt-2 ml-1 text-red-500 font-bold";
                    isIdChecked = false;
                }
            })
	...
	function switchTab(type) {
		...
		// [추가] 로그인 잠금 안내 영역 초기화
	    var lockedArea = document.getElementById('loginLockedArea');
        ...
        if(pwInput) {
            ...
            pwInput.classList.add('focus:border-blue-500');
        }
    ...
    function handleLoginKeyup(event) {
        // 사용자가 누른 키가 'Enter' (키코드 13) 인지 확인
        if (event.key === 'Enter' || event.keyCode === 13) {
            handleLogin(); // 기존 로그인 실행 함수 호출
        }
    }
    ...
    function handleLogin() {
        ...
        if(!userId || !password) { showAlert('아이디와 비밀번호를 입력해주세요.', 'error'); return; }
        
        document.getElementById('loginLockedArea').classList.add('hidden');
        ...
        if(result.success) {
            ...
            // [수정] 계정 잠금 메시지인 경우 별도 UI 노출
            if(result.message.includes("계정이 잠금되었습니다")) {
                document.getElementById('loginLockedArea').classList.remove('hidden');
             	// 2. 입력창 강조 (빨간 테두리 추가)
                var pwInput = document.getElementById('loginPw');
                pwInput.value = "";
                pwInput.classList.add('border-red-500'); // 테두리 빨간색
                pwInput.classList.remove('focus:border-blue-500'); // 파란색 강조 제거
                if(window.lucide) lucide.createIcons(); // 아이콘 렌더링
            } else {
            	showAlert(result.message, 'error');
            }
        }
    })
    .catch(function() { showAlert('로그인 중 오류가 발생했습니다.', 'error'); });
    }
    ...
    function handleRegister() {
        ...
        if (!isIdChecked || userId !== checkedId) {
            showAlert("아이디 중복 확인을 진행해주세요.", 'error');
            return;
        }
        
     	// [추가] 2. 이메일 중복 확인 체크
        if (!isEmailChecked || email !== checkedEmail) {
            showAlert("이메일 중복 확인을 진행해주세요.", 'error');
            return;
        }
        ...
        if (!pwRegex.test(password)) { 
            showAlert("비밀번호는 8~20자 이내이며, 영어 대/소문자, 숫자, 특수문자를 모두 포함해야 합니다.", 'error'); 
            document.getElementById('regPw').focus();
            return; 
        }
        if (password !== pwConfirm) { showAlert("비밀번호 확인이 일치하지 않습니다.", 'error'); return; }

        if (!userName) { showAlert("담당자 이름을 입력해주세요.", 'error'); return; }
        if (!position) { showAlert("직급을 입력해주세요.", 'error'); return; }
        
        if (!phone) { showAlert("휴대폰 번호를 입력해주세요.", 'error'); return; }
        ...
        var zipCode = document.getElementById('zipNo').value;
        var addrKor = document.getElementById('addrRoad').value;
        ...
        if (selectedUserType === 'MASTER') {
            ...
            finalCompanyPhone = document.getElementById('ownerCompanyPhone').value.trim();
        ...
        .then(function(result) {
        	if (result.success) {
                showAlert('회원가입 신청이 완료되었습니다.\n관리자 승인 후 로그인이 가능합니다.', 'success');
                setTimeout(() => { switchTab('login'); }, 2200);
            } else {
                showAlert(result.message || '가입 처리 중 오류가 발생했습니다.', 'error');
            }
        })
        .catch(function(err) {
            console.error(err);
